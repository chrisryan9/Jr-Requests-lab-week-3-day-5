...
const baseURL2 = 'http://localhost:3000/repeat/'

const repeatMyParam = () => {
    //YOUR CODE HERE

    axios.get(`${baseURL2}Chris Ryan`).then(response => {
        console.log(response.data);
        let repeat2 = document.getElementById('repeat-text');
        repeat2.textContent = response.data;
...
const queryURL = 'http://localhost:3000/query-test';

const sendQuery = () => {
    axios.get(`${queryURL}/?did this work?`).then(response => {
        console.log(response.data);
    }).catch(error => console.log(error));
} 
...
/*
    ...
    console says that an empty query was sent.

    2: Send more than 1 query on the URL -- what happened?

    console just returns an error if more than 1 are sent.
*/
